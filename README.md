...
- `redline_core.resolve` — `ResolveAdapter` interface, a real adapter (`connect()` verified against a live, running DaVinci Resolve Studio 21.0.3 instance; project/media/timeline/render calls still stubbed, see Phase 1 note below), and a `MockResolveAdapter` used by all unit tests
...
`ResolveScriptAdapter.connect()` has been verified against the real instance.
Still open: implementing the remaining `ResolveScriptAdapter` methods
(`duplicate_project`, `import_media`, `build_timeline`, `add_markers`,
