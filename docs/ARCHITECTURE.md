...
| **Backup Manager** (Mission 1A) | System-of-record backup + verification only: snapshots `redline.db` via SQLite's Online Backup API (never a raw copy) and the exact required config files into a sealed, hash-verified, atomically-published package. Never contacts Resolve. See `docs/BACKUP_RECOVERY_ARCHITECTURE.md`. | DB path (independent connection, not the shared `Database` instance), Config, Filesystem |
| **Restore Manager** (Mission 1B-A1) | HEALTHY_SOURCE restore only: fresh target re-verification, exact application-schema compatibility via a disposable `Database.init_schema()` comparison DB, a mandatory pre-restore safety backup, quiescence probe + itemized attestations, SQLite sidecar gate, same-volume staging, non-atomic two-step config replacement, an immutable restore transaction journal, and ordered post-restore verification. DEGRADED_SOURCE/MISSING_SOURCE recovery (Mission 1B-A2) is not implemented on `RestoreManager` itself — that lives in `execute_recovery()`, a separate free function, not a `RestoreManager` method. Mission 1B-B's read-only MCP planning surface is implemented and checkpointed locally: `restore_plan` delegates to `RestoreManager.restore_plan()`, while `restore_recovery_plan` delegates to `build_recovery_plan()`; it is not yet published — see `docs/BACKUP_RECOVERY_ARCHITECTURE.md` §17. Never contacts Resolve. See `docs/BACKUP_RECOVERY_ARCHITECTURE.md` §13. | Backup Manager, DB path, Config path, Filesystem |
