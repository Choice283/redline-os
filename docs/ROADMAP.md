...
| 10 — Render Automation | Real Resolve integration for the render methods still stubbed in Phase 6. | Complete — Mission 14 (real `ResolveScriptAdapter.queue_render()`, enqueue-only), Mission 15 (real `ResolveScriptAdapter.get_render_status()`), and Mission 16 (real `ResolveScriptAdapter.cancel_render()`) are complete and live-verified. A further, genuinely new capability — `ResolveScriptAdapter.start_render()`/`RenderManager.start_render()`/CLI `render start <job_id>` — was constructed afterward to close the remaining `render queue`→`render start` gap; unlike Missions 14–16, **`start_render()` has been unit-tested against `MockResolveAdapter` only and has not yet been live-verified.** Its first construction (Rev1) was independently reviewed and returned REVISION REQUIRED (10 findings, 8 blocking); a Rev2 correction pass resolved all ten and was architecturally accepted but not yet approved for publication or live execution (7 further findings, 4 blocking); a Rev3 correction pass resolved all seven and had its overall architecture and safety model ACCEPTED, with one narrow BLOCKING integration mismatch found against live getter-only evidence (Resolve's `OutputFilename` includes the extension); a Rev4 correction pass resolved it, still offline/construction-only. See `docs/ARCHITECTURE.md` §3.8 and `docs/RENDER_START_PATH_CONSTRUCTION.md` §6/§7/§8. |
...
- Status: **Two distinct threads, not one.** The `RLC-E9001` disposable
  queue-acceptance experiment (Missions 39D/39E) remains **open and BLOCKED**
  — `AddRenderJob()` returned an empty result across three controlled live
  attempts and no further attempt is authorized. Separately, the
  `RLC-E9901` production render lifecycle has since been externally
  evidenced as **complete** (queue acceptance, independent Resolve-side
  confirmation, production `start_render()`, render completion, and a
  verified rendered master) — see the dated entry at the end of this
  section and `docs/REDLINE_OS_V1_RELEASE_CANDIDATE.md` §3/§4 for the full
  evidence record. **Neither status blocks Redline OS V1**: RLC-E9901's
  completion is V1's production proof, and RLC-E9001's unresolved
  experiment is documented as a separate, non-blocking deferred thread.
  Do not read "Phase 14 open and BLOCKED" below as describing RLC-E9901 —
  every entry below predates the RLC-E9901 evidence and describes the
  state as understood before it, except where the closing entry says
  otherwise.
...
- A subsequent RLC-E9901 final-ignition tool-selection review traced the real, live-captured RLC-E9901 rev3 queue-snapshot evidence (`RLC-E9901_render_queue_snapshot_rev3_20260810T233837Z.json` / `RLC-E9901_render_queue_comparison_rev3_20260810T234031Z.json`, `exact_single_job_match`) back to this exact probe and found its one remaining gap: the target job's own `JobStatus`, needed to prove `Ready` before any future, separately authorized `StartRendering()`, which `GetRenderJobList()` does not return. Corrected in Rev4 (`EXECUTION_REVISION_ID = phase14.2-render-queue-snapshot-construction-rev4`, replacing Rev3's now-rejected `...-rev3` alongside the already-rejected `...-rev1`/`...-rev2`), documented in full in `docs/PHASE14_RENDER_QUEUE_SNAPSHOT_CONTRACT.md` §11. Adds exactly one new getter, `GetRenderJobStatus` (allowlist: six methods → seven), called by `snapshot` (live collection) once per identified queue entry only, after the existing pre/post drift check and final identity/rendering guard; `compare` (offline evaluation) gains zero new Resolve calls, preserving the `snapshot`/`compare` boundary exactly. A malformed/missing/falsy `GetRenderJobStatus` response fails the entire snapshot collection closed rather than publishing a placeholder, mirroring the existing `job_id`/`job_id_status` contract. `compare` gains one new optional, additive, backward-compatible flag, `--expected-job-status`, evaluated entirely offline against already-captured evidence. The production `redline render status` path was considered and rejected as an alternative source for `JobStatus`: it has a reachable, data-dependent SQLite write branch, not a statically-provable-zero-SQLite-write property. Rev8 (`phase14_resolve_context_snapshot.py`) remains untouched and byte-identical; no production render-start code changed. **Rev4 has not itself been independently reviewed or approved, and has not been executed live** — only mocked unit tests exercise the new getter. The focused test suite grew from 166 to 231 tests; the render/Phase14/RLC-E9901-focused regression is a clean 1176/1176; the full `tests/unit` regression is 2283 passed, 25 failed (all 25 independently confirmed pre-existing and unrelated — 24 already-documented Windows-temp-path/YAML-escaping CLI fixture failures plus one network-dependent pip-bootstrap smoke test), 9 skipped. `git diff --check` exits 0; only the two intended files plus this documentation changed. This does not change Phase 14's BLOCKED status, does not claim RLC-E9901 independent render-queue-plus-status closure, and does not authorize live execution. See `docs/CHANGELOG.md` for full correction detail.
- **Independent Rev4 source review completed and returned a publication-blocking finding**: `POST_STATUS_REBRACKET_REQUIRED` (`REV4_REQUIRES_CORRECTION`). The review traced the actual Rev4 live call sequence from source and found that `GetRenderJobStatus()` ran *after* the pre-existing "final" identity/rendering guard, and nothing re-established project identity, timeline identity, `rendering == false`, or queue stability *after* those calls before evidence was published — a published Rev4 snapshot could theoretically combine a `rendering=false` observation from before the status-fetch window with a `JobStatus=Ready` observation from during/after it, without proof the two were ever simultaneously true. The same review independently reproduced the Rev4 construction's own claims (repository/diff scope, probe hash, zero reachable Resolve mutations, zero SQLite writes, all six `--expected-job-status` semantics, historical Rev3 evidence rejection as intentional and documented) and confirmed the 25th broad-suite failure as pre-existing/environmental, not a Rev4 regression (24/2283 reproduced after excluding the network-sensitive pip-bootstrap test). Corrected in Rev5 (`EXECUTION_REVISION_ID = phase14.2-render-queue-snapshot-construction-rev5`, replacing Rev4's now-rejected `...-rev4` alongside the already-rejected `...-rev1`/`...-rev2`/`...-rev3`), documented in full in `docs/PHASE14_RENDER_QUEUE_SNAPSHOT_CONTRACT.md` §12. Adds a new post-status rebracket (`_post_status_rebracket()`) that re-verifies context/rendering **and** re-reads the render queue a third time, requiring it to equal the already drift-verified queue exactly (`post_status_queue_drift` otherwise) — Option B over context/rendering-only, because the queue's own identity claims are exactly what `JobStatus` is meant to be combined with for one simultaneous final-ignition proof. No new Resolve method introduced: the allowlist remains the same seven methods, `GetRenderJobList` simply read a third time. Published evidence now uses the post-status guard's fresh values, never the earlier pre-status guard's. `GetRenderJobStatus()` itself, `compare`'s offline-only boundary, `--expected-job-status`, Rev8, and all production render-start code are unchanged. **Rev5 has not itself been independently reviewed or approved, and has not been executed live.** The focused test suite grew from 231 to 249 tests (6 new adversarial temporal-rebracket scenarios); the render/Phase14/RLC-E9901-focused regression is a clean 1194/1194; the full `tests/unit` regression (network-sensitive test deselected per its established non-determinism) is 2301 passed, 24 failed — the same 24 pre-existing failures by name — 9 skipped. `git diff --check` exits 0. This does not change Phase 14's BLOCKED status and does not authorize live execution. See `docs/CHANGELOG.md` for full correction detail.
- **RLC-E9901 production render lifecycle: external evidence reconciled, 2026-08-11.** Every entry above this one predates and was superseded by a live production render executed the same day the last entry above was published. At repository checkpoint `0a0614bbb90af64b51766a434c920291ce2f027b` ("feat: add render job status to Phase 14 queue snapshot probe"), Paul Jones authorized one production `render start` for the already-queued RLC-E9901 render job (Resolve Job ID `3c0af847-bddd-43ee-8b79-a7b64cb915b4`). Immediately before the start, the Rev5 render-queue snapshot/comparison probe independently captured Resolve's own queue state and classified it `exact_single_job_match` — the independent Resolve-side confirmation earlier entries above describe as "still pending" was obtained at that point. `python -m cli.main render start 1` then ran the real production path (`RenderManager.start_render()` → `ResolveAdapter.start_render()` → `Project.StartRendering(...)`), exit code `0`, exactly once, no retry. A subsequent getter-only `render status 1` call reconciled the job to `complete` and the episode to `rendered`. This documentation-correction mission independently re-verified, read-only, outside this repository: all four evidence files exist at their recorded paths under `C:\Users\pj198\RedlineOSLive\` with SHA-256 hashes matching exactly; the rendered master `RLC-E9901_MASTER.mov` exists, is `132,364,925` bytes, and hashes to `17e0099b591acd30790bbf3520955ba51f645b3f303ec8ff980219242230b6e9`; and the live `redline.db` at that same evidence location shows `render_jobs` row `id=1` with `status=complete` and `episodes` row `RLC-E9901` with `status=rendered`, both matching the evidence bundle exactly. **This closes RLC-E9901's own queue-acceptance-and-render objective; it does not touch, resolve, or retroactively change the separate `RLC-E9001` disposable experiment's documented failures above**, which remain open and BLOCKED as their own thread. One provenance gap is noted, not concealed: the exact original artifact showing how/when RLC-E9901's `AddRenderJob()` queue acceptance was first produced was not re-traced by this correction — only the later queue-confirmation-through-completion chain was independently re-verified. See `docs/REDLINE_OS_V1_RELEASE_CANDIDATE.md` §4 for the complete evidence table and `docs/CHANGELOG.md` for this correction's own entry.
---
...
- `docs/REDLINE_OS_V1_RELEASE_CANDIDATE.md` — the V1 closure record: V1 status, CI exception classification, and deferred V2 work.
