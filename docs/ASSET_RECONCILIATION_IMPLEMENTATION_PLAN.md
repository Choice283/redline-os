...
| `evidence.py` | Plan-local evidence and evidence builder contracts. | `PlanEvidence`, `EvidenceCandidate`, `EvidenceSemanticRef`, `EvidenceBuilder`. | `_semantic_key`, `_merge_timestamps`, `_redacted_value`. | `enums`, `subjects`. | Final plan assembly outside evidence IDs; public serialization. | `test_evidence.py` |
| `actions.py` | Inert action payloads and factories. | Tagged action dataclasses, `ActionDraft`, `ActionFactory`. | `_require_subject_compatible`, `_action_sort_key`. | `enums`, `subjects`, `evidence`, `findings`. | Executing actions or writing SQLite. | `test_actions.py` |
| `findings.py` | Structured finding codes, templates, factories, ranking. | `ReconciliationFinding`, `FindingFactory`, `FindingCode`, `FindingTemplate`. | `_sanitize_message`, `_finding_sort_key`. | `enums`, `subjects`, `evidence`. | Free-form matching prose, logging secrets. | `test_findings.py` |
...
| `planner.py` | Orchestrate pure planning and invariant checks. | `ReconciliationPlanner`, `plan_reconciliation`. | `_assemble_items`, `_assign_ids`, `_validate_plan_invariants`. | All domain modules except `serialization`. | Repository calls, filesystem calls, action execution. | `test_planner.py` |
