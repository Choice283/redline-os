...
Redline OS's MCP server (`src/mcp_server`) exposes the Episode/Asset/Media/Timeline/Render/Archive managers (Phases 2-7, Archive extended by Phase 15) as 20 MCP tools - the full pipeline described in `docs/ARCHITECTURE.md` section 4, from "create an episode" through "archive it once it's rendered, and verify that archive later." (This intro previously undercounted these as "19" - a pre-existing, internal inconsistency with this same document's own "Verified" section, which already said 20; corrected here, as source-derived truth, while adding Mission 1B-B below.) Mission 1B-B adds 4 more, read-only Backup/Restore/Recovery planning tools - 24 tools total.
...
| `archive_recover` | `episode_id: str, archive_id: str` | Phase 15 Mission 15H. Registers an already-published, independently-verified final package left `VERIFIED_UNREGISTERED` by a prior `archive_create` call — never repairs, rebuilds, or re-seals a package. `archive_id` is required and explicit; there is no scan/discovery mode, no arbitrary package path, no force flag, no repair mode. Verifies the sealed package with the same published verifier `archive_verify` uses, reads only its already-sealed `episode.json`/`render_job.json` restore metadata (never re-reads current source/evidence/config), cross-checks against current DB state, and — if every precondition holds — performs the exact same guarded `commit_verified_archive()` transaction `archive_create` itself would have performed. Idempotent: calling it again after a successful recovery returns `classification: "already_registered"`, never a duplicate row. A DB-commit failure during recovery is reported the same way as `archive_create`'s own (`classification: "verified_unregistered"`) — the package remains recoverable. |

### Backup / Restore / Recovery (read-only — Mission 1B-B)

Mission 1B-B exposes exactly four read-only inspection/planning tools over the completed Mission 1A (Backup)/1B-A1 (Restore)/1B-A2 (Recovery) capabilities. These are bound to a second, independent `RestoreContext` (`mcp_server.context.build_restore_context()`), never the primary `AppContext` — see `docs/BACKUP_RECOVERY_ARCHITECTURE.md`'s Mission 1B-B section for the full composition rationale.

**No mutating Backup/Restore/Recovery capability is exposed here, and none ever will be under the current authorization model.** `backup_create`, `RestoreManager.restore()`, and `execute_recovery()` are permanently out of scope: the last two require an itemized `mcp_stopped` operator attestation (part of `QuiescenceAttestations`/`RecoveryAuthorization`) that no call arriving through a running MCP server can ever truthfully assert — see `docs/BACKUP_RECOVERY_ARCHITECTURE.md` for the full analysis. No code in this server may fabricate that attestation.

| Tool | Args | Returns |
|---|---|---|
| `backup_list` | - | Every sealed Mission 1A backup on disk, newest first (read-only). → `BackupManager.list_backups()`. |
| `backup_verify` | `backup_id: str` | Independently re-verifies a sealed backup at rest — manifest hash, payload hashes, `PRAGMA integrity_check` against the backup's own database copy. Never touches the live database. → `BackupManager.verify_backup()`. |
| `restore_plan` | `backup_id: str` | Read-only preview of whether `restore(backup_id)` would currently be able to proceed (HEALTHY_SOURCE only). Creates no pre-restore safety backup, stages nothing, replaces nothing. → `RestoreManager.restore_plan()`. |
| `restore_recovery_plan` | `backup_id: str` | Read-only classification of whether a DEGRADED_SOURCE/MISSING_SOURCE recovery against `backup_id` would be architecturally eligible to proceed. Creates no degraded-source capture, no journal, no disposition. → `build_recovery_plan()`. |

Both planning tools (`restore_plan`/`restore_recovery_plan`) are deliberately "non-raising, report why not": every domain-expected blocking condition (target backup didn't verify, schema incompatible, live database not quiescent, sidecar present, source degraded/missing) is returned as data (`blocking_issues`, `would_proceed: false`), never an exception. Only a malformed `backup_id` (failing `validate_backup_id()`) produces `{"success": false, "error": "..."}`.

## Verified

`create_server(use_mock_resolve=True)` has been smoke-tested end-to-end: server construction, `list_tools()` (all 24 tools), and real `call_tool()` round-trips for `create_episode`, `list_episodes`, `verify_assets_for_episode`, and `queue_render` all work through the real `mcp` package (not just the underlying `_*` functions). `tests/unit/test_installed_mcp_startup_smoke.py` additionally proves, against a real installed wheel, that `create_server(use_mock_resolve=True)` registers exactly this 24-tool set, including all four Mission 1B-B tools. Phase 10 live verification completed the real Resolve render adapter lifecycle (`queue_render`, `get_render_status`, and `cancel_render`); use mock Resolve for MCP startup checks and real Resolve only for workflows that intentionally touch Resolve state.
