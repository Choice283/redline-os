...
class RestoreManager:
    ...
    def _verify_restore(self, *, manifest: dict, backup_id: str) -> None:
        """Thin wrapper (Mission 1B-A2-3 extraction) around the shared,
        module-level ``redline_core.restore.verification.verify_restore()``
        -- the exact same STEP 0-6 sequence this method always ran,
        unchanged, so Mission 1B-A2-3's recovery execution calls the
        identical verification, never a duplicated or approximated copy.
        STEP 7 (``VERIFIED_SUCCESS``) is recorded by the caller
        (``restore()``) once this method returns without raising."""
        verify_restore(
