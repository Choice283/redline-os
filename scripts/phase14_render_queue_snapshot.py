...
* Every dynamically dispatched Resolve method is restricted to a closed,
  six-method, getter-only allowlist (``READ_ONLY_RESOLVE_METHODS``),
  unchanged from Rev1.
...
# verify current timeline identity, verify rendering is not active, and read
# the render queue. See the module docstring for why this is not imported
# from Rev8's own, broader allowlist. Unchanged from Rev1.
# ---------------------------------------------------------------------------
...
def collect_render_queue_snapshot(resolve: Any, context: QueueSnapshotContext) -> dict[str, Any]:
    ...
    Live call sequence (Rev2, Finding 5)::
        ...
        verify expected current project/timeline again
        IsRenderingInProgress -> false

        require normalized A == normalized B

    The third verification performs no further queue read; its observed
    project/timeline/rendering values are the ones published in the
    returned snapshot, since they are the most recently confirmed-consistent
    state.
    """
    ...
    # final_project_name == context.expected_project (and likewise
    # timeline_name/rendering) always holds by construction once all three
    # calls succeed. The render queue itself carries no such external
    # expected value, so it is the one field that can genuinely drift
    # between the two reads; comparing its normalized entries also
    # transitively covers any count change.
    ...
    # Rev2, Finding 5: a third context/rendering guard, with no further
    # queue read, strictly after the second GetRenderJobList() call
    # returns. Rev1 had no check at all after the final queue read, so
    # rendering could begin — or the current project/timeline could change
    # — in the window between B returning and evidence being finalized,
    # without ever being observed.
    _final_project, final_project_name, final_timeline_name, final_rendering = (
    ...
    snapshot = {
        ...
        "render_queue": post["render_queue"],
...
_REQUIRED_ENTRY_KEYS = frozenset({"index", "job_id", "job_id_status", "fields"})
...
def compare_expected_job_id(snapshot: Mapping[str, Any], expected_job_id: str | None) -> dict[str, Any]:
    """Offline-only acceptance comparison against one previously collected,
    fully validated snapshot.

    Makes zero Resolve contact. Rev2, Finding 1: every outcome is one
    explicit, mutually exclusive classification — a caller never has to
    infer success by combining multiple loosely related fields the way
    Rev1's ``has_unexpected_additional_jobs``/``exactly_one_matching_job``
    pair required. Exactly one classification,
    ``"exact_single_job_match"``, means formal single-job closure:
    ``render_queue_count == 1 and identified_job_id_count == 1 and
    matching_job_count == 1`` (which together already imply zero
    unidentified entries and zero other identified jobs, since the queue
    has exactly one entry total). Every other classification is
...
def run_compare_command(args: argparse.Namespace) -> int:
    ...
    comparison = compare_expected_job_id(snapshot, args.expected_job_id)
    write_strict_json_no_overwrite(args.output, comparison)
    if args.expected_job_id is None:
        return 0
    return 0 if comparison["classification"] == "exact_single_job_match" else 3
